...
function chekideh_register_plugin_options_metabox() {
    ...
    $cmb_general_options->add_field( array(
        ...
        'default' => 'لطفاً نظرات کاربران که در پایان درج شده را بررسی کرده و یک خلاصه جامع از آن‌ها ارائه دهید. در این خلاصه، به نکات مثبت و منفی اصلی که کاربران مطرح کرده‌اند اشاره کنید و همچنین هرگونه الگوی مشترک یا تم‌های مهمی که در نظرات وجود دارد را شناسایی نمایید. هدف از این خلاصه، ارائه‌ی دیدگاهی کلی و مفید درباره‌ی تجربیات کاربران است. خروجی باید به صورت HTML و بدون درج triple backticks باشد. فقط از تگ‌های p ul li استفاده کن.',
        'type'    => 'textarea',
    ) );
...
function chekideh_add_comments_summary() {
    ...
    echo '<div class="chekideh-comments-summary">' . $summary . '</div>';
}

function chekideh_generate_summary($transient_key, $comments) {
    $chekideh_summary_prompt = 
        cmb2_get_option( 'chekideh_plugin_options', 'openai_comments_summary_prompt', 'لطفاً نظرات کاربران که در پایان درج شده را بررسی کرده و یک خلاصه جامع از آن‌ها ارائه دهید. در این خلاصه، به نکات مثبت و منفی اصلی که کاربران مطرح کرده‌اند اشاره کنید و همچنین هرگونه الگوی مشترک یا تم‌های مهمی که در نظرات وجود دارد را شناسایی نمایید. هدف از این خلاصه، ارائه‌ی دیدگاهی کلی و مفید درباره‌ی تجربیات کاربران است. خروجی باید به صورت HTML و بدون درج triple backticks باشد. فقط از تگ‌های p ul li استفاده کن.')
        . ' لیست دیدگاه‌ها:'
        . $comments;
    ...
    set_transient($transient_key, $summary->data, 600);
